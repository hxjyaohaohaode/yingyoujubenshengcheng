...
STORY_PLANNER_SKILL = Skill()
STORY_PLANNER_SKILL.name = "story_planner"
STORY_PLANNER_SKILL.intent = "create.plan"
STORY_PLANNER_SKILL.model = "ds-reasoner"
STORY_PLANNER_SKILL.prompt_template = """基于用户设定生成完整的Story Plan（故事蓝图），作为后续所有生成步骤的全局约束锚点。
...
输出严格JSON格式，包含以下7个核心字段：
{{
  "core_logline": "核心梗概（1-2句话，概括整个故事的核心冲突和走向）",
  "theme_statement": "主题声明（一个道德悖论式命题，如'忠诚与正义能否共存'）",
  "character_arcs": [
    {{
      "character_name": "角色名",
      "arc_start": "起点状态",
      "arc_turning_point": "转折事件",
      "arc_end": "终点状态",
      "interlocks_with": ["与哪个角色弧线互锁"]
    }}
  ],
  "plot_nodes": [
    {{
  ...
  "foreshadow_routes": [
    {{
      ...
      "plant_chapter": "埋设章节",
      "reinforce_chapters": ["强化章节1", "强化章节2"],
      "reveal_chapter": "揭示章节",
      "expected_emotional_effect": "预期情感效果"
    }}
  ],
  "emotion_curve_plan": [
    {{
      "chapter": 1,
      "emotion_target": "紧张/温暖/悲伤/震撼/释然",
      "intensity": 7,
      "note": "本章情感目标说明"
    }}
  ],
  "choice_philosophy": "选择哲学说明（道德灰度原则、低分支高情感范式等）",
  "recommended_chapter_count": 10
}}"""
STORY_PLANNER_SKILL.output_parser = lambda text: _parse_story_plan(text)


def _parse_story_plan(text: str) -> dict:
    code_block = re.search(r"```json\s*(.*?)\s*```", text, re.DOTALL)
    if code_block:
        try:
            result = json.loads(code_block.group(1))
            if isinstance(result, dict) and "core_logline" in result:
                return result
        except (json.JSONDecodeError, TypeError):
            pass
    brace_match = re.search(r"\{.*\}", text, re.DOTALL)
    if brace_match:
        try:
            ...
            pass
    return {"core_logline": text[:200], "theme_statement": "", "character_arcs": [], "plot_nodes": [], "foreshadow_routes": [], "emotion_curve_plan": [], "choice_philosophy": "", "recommended_chapter_count": 0}
...
class OrchestratorAgent(BaseAgent):
    ...
    skills = {
        ...
        "story_planner": STORY_PLANNER_SKILL,
    }
    ...
    async def execute(self, task: AgentTask) -> AgentResult:
        ...
        if task.task_type == "story_planner":
            return await self._handle_story_planner(task)
    ...
    async def _handle_story_planner(self, task: AgentTask) -> AgentResult:
        from services.llm_prompts import build_story_plan_prompt
        ...
        if layer0:
            genre = genre or layer0_value(layer0, "genre", "")
        ...
        try:
            response: ModelResponse = await self.gateway.invoke(
                intent="create.plan",
                messages=[
            ...
            parsed = _parse_story_plan(raw_text)
            return AgentResult(
                status="completed",
                data=parsed,
            )
        except Exception as e:
            logger.error("Story Plan生成失败: %s", str(e))
            return AgentResult(
                status="failed",
                data={"error": str(e)[:200]},
                issues=[f"Story Plan生成失败: {str(e)[:100]}"],
            )
